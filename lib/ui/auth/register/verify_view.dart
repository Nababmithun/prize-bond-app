            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  Text(tr('verify.title'), textAlign: TextAlign.center,
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        Text(tr('verify.code_label'),
                            style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: cs.onSurface.withOpacity(.85))),
                        const SizedBox(height: 20),

                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: List.generate(6, (i) => _OtpBox(
                            controller: _ctrls[i], focusNode: _nodes[i], onChanged: (v) => _onChanged(i, v),
                          )),
                        ),
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton(
                            onPressed: vm.isLoading ? null : () async {
                              if (_otp.length == 6) {
                                final ok = await vm.verifyOtp(_otp, emailArg: widget.emailFromRouteArg);
                                if (!mounted) return;
                                if (ok) {
                                  // After verify → go to Login (as requested)
                                  Navigator.pushNamedAndRemoveUntil(context, AppRoutes.login, (r) => false);
                                } else {
                                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(vm.error ?? tr('common.retry'))));
                                }
                              } else {
    ...
    return SizedBox(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          counterText: '',
          filled: true, fillColor: const Color(0xFFE6F2E6),
